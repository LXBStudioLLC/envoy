...
/// 4. Build a timeline: usable snapshots + current posting appended last (time =
///    <see cref="JobPosting.LastUpdatedUtc"/> if present, else no span bonus).
...
///    and add +0.10 if the timeline span is &lt;= 30 days (cap 0.75).
...
public class RepostFrequencySignal : IGhostSignal
{
    ...
    public Task<SignalResult?> EvaluateAsync(JobPosting posting, CancellationToken ct = default)
    {
        try
        {
            // ── PATH A: history-backed ───────────────────────────────────────
            var pathA = TryPathA(posting);
            if (pathA != null)
                return pathA;

            // ── PATH B: timestamps only ──────────────────────────────────────
    ...
    private Task<SignalResult?>? TryPathA(JobPosting posting)
    {
        ...
        // Build timeline: snapshots + current posting appended last
        ...
        var currentTime = posting.LastUpdatedUtc ?? DateTime.UtcNow;
        timeline.Add(new TimelineEntry(currentTime, posting.DescriptionText));
        ...
        var (score, confidence) = ComputeScoreAndConfidencePathA(unchangedBumps, timeline);
        var evidence = BuildEvidencePathA(unchangedBumps, timeline);
    ...
    private static (double Score, double Confidence) ComputeScoreAndConfidencePathA(int unchangedBumps, List<TimelineEntry> timeline)
    {
        ...
        var span = timeline[^1].Time - timeline[0].Time;
    ...
    private static string[] BuildEvidencePathA(int unchangedBumps, List<TimelineEntry> timeline)
    {
        var span = timeline[^1].Time - timeline[0].Time;
    ...
    private Task<SignalResult?>? TryPathB(JobPosting posting)
    {
